...
transformationMatrix.compose(new THREE.Vector3(), new THREE.Quaternion(), new THREE.Vector3(1,1,1));

// Sets a transformation matrix, which is globally applied to all polygons in this algorithm.
// Allows for efficient position / rotation of the model without explicit geometry transform.
function setTransformationMatrix(matrix){
    transformationMatrix.copy(matrix);
}

// Sets the transformation matrix, using position and euler rotation inputs.
function setTransformation(x, y, z, rx, ry, rz){
    ...
    transformationMatrix.compose(tempPosition,tempRotation,new THREE.Vector3(1,1,1));
}

// Generates a transform matrix from pos, rot, scale
function generateTransformMatrix(x,y,z,rx,ry,rz,sx,sy,sz){
    var tempMatrix = new THREE.Matrix4();
    var tempPosition =new THREE.Vector3(x,y,z);
    var tempRotation = new THREE.Quaternion();
    tempRotation.setFromEuler(new THREE.Euler(rx * (Math.PI/180),ry * (Math.PI/180),rz * (Math.PI/180)));
    tempMatrix.compose(tempPosition,tempRotation,new THREE.Vector3(sx,sy,sz));
    return tempMatrix;
}

// Takes in a point and transforms it using the matrix, returns the original point instance
function transformPoint(p){
    p.applyMatrix4(transformationMatrix);
    return p;
}
...
function calculateVolumeCenterOfMass(geometry, doClipping, aboveZero){
    ...
    for(var i = 0; i < geometry.faces.length; i++){
        ...
        var P = transformPoint(new THREE.Vector3(geometry.vertices[Pi].x, geometry.vertices[Pi].y, geometry.vertices[Pi].z));
        var Q = transformPoint(new THREE.Vector3(geometry.vertices[Qi].x, geometry.vertices[Qi].y, geometry.vertices[Qi].z));
        var R = transformPoint(new THREE.Vector3(geometry.vertices[Ri].x, geometry.vertices[Ri].y, geometry.vertices[Ri].z));
